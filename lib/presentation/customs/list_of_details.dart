import 'package:flutter/material.dart';

List<String> burgerDataPthSpecial = [
  'Images/special1.png',
  'Images/combo2.png',
  'Images/special3.png',
  'Images/special4.png',
];
List<String> burgerDataDesSpecial = [
  'BurgerB Bacon: Two Beef burgers with onion,tomatoes and tasty bacon 15\$',
  'KIDS MEAL: a small cheeseburger with french fries and soft drink 5\$',
  'FLAMING MEAT: beef burger with piece of spicy roasted chicken  9\$',
  'DOUBLE MEAT BEEF: two beef layers with chicken, lettuce onion and cheese  10\$'
];

List<String> burgerDataPth = [
  'Images/Burger1image.jpg',
  'Images/Burger2image.jpeg',
  'Images/Burger3image.jpeg',
  'Images/Burger4image.jpeg',
];
List<String> burgerDataDes = [
  'BurgerB Classic: Two Beef burgers with melted cheese and onion rings 10\$',
  'CHICKEN CRISPY: piece of chicken with mayo, lettuce and crispy onion 6\$',
  'CHICKEN WRAP: our special chicken wrapped with salad in tortilla bread  7\$',
  'MEAT TACCO: fried meat with tomatoes and onion with spicy foam 12\$'
];

List<String> drinksPth = [
  'Images/coffeePic.jpeg',
  'Images/softPic.jpeg',
  'Images/smoothiePic.jpeg',
  'Images/icePic.jpeg'
];
List<String> drinksDes = [
  'COFFEE: Try out our delicious coffee  3\$',
  'SOFT DRINKS: All types of soft drinks all in one place 2\$',
  'SMOOTHIE:  All smoothie flavors pick what you want 4\$',
  'ICE-CREAM: Try our homemade ice cream flavors 5\$'
];

List<String> hintLabels = ['Name', 'Telephone Number'];

List<String> tabName = ['POPULAR', 'SPECIAL', 'OTHER'];
List<IconData> tabIcon = [
  Icons.people,
  Icons.timelapse,
  Icons.local_drink_rounded
];
